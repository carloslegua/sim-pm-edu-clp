(function() {
	...
	function analyzeWbs(wbs) {
		...
		const totalCost = leafIds.reduce((s, id) => s + Math.max(0, num$1(nodes[id].cost)), 0);
		let complete = 0;
		leafIds.forEach((id) => {
			const n = nodes[id];
			...
			const cost = num$1(n.cost);
			if (cost <= 0) add("D4", id, nm(id) + " no tiene costo estimado.");
			const days = calendarDays(n.start, n.end) ?? Math.max(0, num$1(n.duration));
	...
	function normalizeBoe(raw) {
		...
		TEXT_KEYS.forEach((k) => {
			b.text[k] = str$3(raw[k]);
		});
		b.version = str$3(raw.version) || "1.0";
		b.status = STATUSES.indexOf(raw.status) >= 0 ? raw.status : "borrador";
		b.preparedBy = str$3(raw.preparedBy);
		b.reviewedBy = str$3(raw.reviewedBy);
		b.approvedBy = str$3(raw.approvedBy);
		b.approvedOn = /^\d{4}-\d{2}-\d{2}$/.test(str$3(raw.approvedOn)) ? str$3(raw.approvedOn) : "";
	...
	var strs = (v) => Array.isArray(v) ? v.map(str$2).filter(Boolean) : [];
	function normalizeCr(o, fallbackId) {
		...
		AREAS.forEach((a) => {
			...
			impact[a] = {
				state: ["sin_impacto", "con_impacto"].indexOf(str$2(q.state)) >= 0 ? q.state : "sin_evaluar",
				note: str$2(q.note)
			};
		});
		...
		return {
			...
			requester: str$2(x.requester),
			requestedOn: str$2(x.requestedOn),
			origin: str$2(x.origin),
			type: str$2(x.type),
			impact,
			...
			scheduleBaseline: str$2(x.scheduleBaseline),
			status: CR_STATUSES.indexOf(x.status) >= 0 ? x.status : "Pendiente",
			decidedOn: str$2(x.decidedOn),
			approver: str$2(x.approver),
			authLevel: str$2(x.authLevel),
			sponsorAuth: !!x.sponsorAuth,
			rationale: str$2(x.rationale),
			implementedOn: str$2(x.implementedOn),
			notes: str$2(x.notes)
		};
	}
	...
	function areaRows(f) {
		...
		return [
			...
			},
			{
				...
				file: null,
				state: "vacio",
				metric: "sin módulo en la suite",
				note: "Aún no hay módulo: no se inventa su contenido.",
				unavailable: true
			},
			{
				key: "comms",
	...
	function buildCtx() {
		...
		try {
			...
			const cp = portfolio$1(arr(rec(G.getModule("changes")).requests).map((o, i) => normalizeCr(o, "cr" + (i + 1))), {
				...
				mods: arr(rec(req).changes).map((x) => ({
					id: str(x.id),
					code: str(x.code || x.id),
					title: str(x.summary || x.title)
				})),
				risks: risks.map((r) => ({
					id: r.id,
					code: r.code,
    ...
    <div class="grid2">
      ...
      <div class="card"><h3>Aprobación del plan</h3>
        ...
        ${approved ? `<p class="small">${stale ? "⚠ <b>Plan aprobado desactualizado</b>: cambió una línea base después de la aprobación." : "✔ <b>Plan aprobado</b> v" + esc(plan.version) + " por " + esc(plan.approvedBy) + " el " + esc(plan.approvedOn) + "."}</p><button class="btn" id="btnNewVersion">＋ Nueva versión del plan</button>` : `${blockers.length ? `<p class="small" style="color:#a05a00">No se puede aprobar todavía: ${esc(blockers.join("; "))}.</p>` : ""}<button class="btn primary" id="btnApprove"${blockers.length ? " disabled" : ""}>✔ Aprobar el plan</button>`}
	...
	function approve() {
		...
		showConfirm("Se aprobará el plan v" + plan.version + " con las líneas base actuales" + (finds.length ? " (quedan " + finds.length + " aviso(s) de integración sin resolver)" : "") + ". Si luego cambian, quedará desactualizado. ¿Continuar?", "Aprobar el plan", "Aprobar").then((ok) => {
			...
			plan.snapshot = snapshotOf(f);
			ctxDirty = true;
			save();
			renderState();
			renderDoc();
			setStatus("Plan v" + plan.version + " aprobado.");
		});
	}
	function newVersion() {
	...
	function buildSections() {
		...
		}
		{
			...
			h += `<h3 class="d2" id="s10-ap">Aprobación del plan</h3>` + kv([
				...
				["Estado", plan.status === "aprobado" ? "Aprobado" : "Borrador"],
	...
	function renderDoc() {
		const C = getCtx();
		$("docView").innerHTML = C.connected ? docHtml() : "<div class=\"empty-hint\">Sin proyecto activo: no hay nada que documentar.</div>";
	}
	...
	function exportWord() {
		...
		a.href = url;
		a.download = "plan_para_la_direccion_v" + plan.version + ".doc";
