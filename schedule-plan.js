(function() {
	...
	function defaultThresholds() {
		return [{
			key: "SV",
			metric: "SV — Variación de Cronograma",
			unit: "%",
			higherIsBetter: true,
			greenValue: -5,
			redValue: -10,
			...
			key: "SPI",
			metric: "SPI — Índice de Desempeño del Cronograma",
			unit: "",
			higherIsBetter: true,
			greenValue: .95,
			redValue: .9,
	...
	function sampleState() {
		return {
			...
			controlThresholds: [
				{
					...
					greenValue: -5,
					redValue: -10,
					action: "Analizar ruta crítica y activar acciones correctivas del paquete afectado.",
					fixed: true
				},
				{
					key: "SPI",
					...
					greenValue: .95,
					redValue: .9,
					action: "Escalar al comité de control; evaluar re-secuenciamiento o recursos adicionales.",
					fixed: true
				},
				{
					key: "HOLGURA",
					metric: "Consumo de holgura en ruta casi crítica",
					unit: "%",
					higherIsBetter: false,
					greenValue: 40,
					redValue: 70,
					action: "Revisar dependencias y considerar fast-tracking o crashing.",
					fixed: false
				}
			],
			performanceMeasurement: {
			...
			milestones: [
				{
					...
					type: "interno",
					constraint: "FNLT",
					notes: "Cierra la fase de Dirección de Proyecto (línea base inicial)."
				},
				{
					...
					type: "regulatorio",
					constraint: "FNET",
					notes: "Habilita el inicio de movimiento de tierras."
				},
				{
				...
				},
				{
					...
					type: "contractual",
					constraint: "FNLT",
					notes: "Fin de Pruebas y Puesta en Marcha; cierre contractual con el cliente."
				}
			],
			scheduleReserve: {
				pct: 8,
			...
			roles: [
				{
					...
					person: "",
					responsibility: "Aprueba la línea base del cronograma y los cambios que excedan el umbral de rebaselinado.",
					raci: "A"
				},
				{
					role: "Director de Proyecto",
					person: "",
					responsibility: "Es responsable integral del cronograma: aprueba actualizaciones, autoriza reserva y escala variaciones fuera de umbral.",
					raci: "R"
				},
				{
					role: "Programador / Controlador de Cronograma",
					person: "",
					responsibility: "Mantiene el modelo de programación, calcula la ruta crítica, actualiza avances y emite reportes.",
					raci: "R"
				},
				{
					role: "Responsables de paquete de trabajo",
					person: "Según Matriz RACI (EDT × OBS)",
					responsibility: "Reportan avance físico semanal de sus paquetes asignados.",
					raci: "R"
				}
			],
			reportingFormats: [
				{
					name: "Curva S (PV / EV / AC)",
					frequency: "Quincenal",
					audience: "Comité de Obra / Sponsor",
					tool: "Simulador EVM + CPM del curso"
				},
				{
					name: "Diagrama de Gantt actualizado",
					frequency: "Semanal",
					audience: "Equipo del proyecto",
					tool: "Herramienta de programación"
				},
				{
					name: "Informe de variaciones (SV, SPI, ruta crítica)",
					frequency: "Quincenal",
					audience: "Sponsor / Cliente",
					tool: "Plantilla de informe de control"
				}
			],
			changeControl: {
			...
			exclusions: ["No se incluye la programación detallada de actividades de operación posteriores a la entrega (fuera del alcance del proyecto).", "No se modelan cronogramas de proyectos paralelos de DISTRIB+ S.A. fuera del almacén de Lurín."],
			approval: {
	...
	function normalizeState(s) {
		...
		if (!Array.isArray(s.calendar.workingTimes) || !s.calendar.workingTimes.length) s.calendar.workingTimes = [{
			from: "08:00",
			to: "12:00"
		}, {
			from: "13:00",
		...
		let arr = Array.isArray(s.controlThresholds) ? s.controlThresholds : [];
		arr = arr.map((t) => {
			if (t && (t.greenValue != null || t.redValue != null)) return t;
			return {
				key: t && t.key,
				metric: t && t.metric || "Métrica",
				unit: t && t.unit || "",
				higherIsBetter: true,
				greenValue: 0,
				redValue: 0,
		...
		["SV", "SPI"].forEach((key) => {
			...
			if (!found) arr.push(deepClone(def));
				...
				found.fixed = true;
				found.higherIsBetter = true;
				if (!found.metric) found.metric = def.metric;
				if (found.unit == null) found.unit = def.unit;
			}
		});
		const order = {
			SV: 0,
		...
		arr.sort((a, b) => {
			return (order[a.key] != null ? order[a.key] : 99) - (order[b.key] != null ? order[b.key] : 99);
		});
	...
	function renderEditableTable(container, rows, columns, opts) {
		opts = opts || {};
		...
		function draw() {
			...
			if (!rows.length) html += "<tr><td colspan=\"" + (columns.length + 1) + "\" style=\"color:var(--ink-2); font-size:12px; padding:12px 10px;\">Sin filas todavía. Usa \"" + (opts.addLabel || "+ Agregar fila") + "\".</td></tr>";
			rows.forEach((r, idx) => {
				html += "<tr>";
				columns.forEach((c) => {
					const val = r[c.key] == null ? "" : r[c.key];
					if (c.type === "select") html += "<td><select data-idx=\"" + idx + "\" data-key=\"" + c.key + "\">" + (c.options || []).map((o) => "<option value=\"" + esc(o.value) + "\"" + (o.value === val ? " selected" : "") + ">" + esc(o.label) + "</option>").join("") + "</select></td>";
			...
			container.querySelectorAll("[data-idx]").forEach((el) => {
				el.addEventListener("input", () => {
					const input = el;
					const idx = +input.dataset.idx, key = input.dataset.key;
					rows[idx][key] = input.type === "number" ? Number(input.value) : input.value;
					onChange();
				});
			});
	...
	function renderSimpleList(container, arr, opts) {
		opts = opts || {};
		const onChange = opts.onChange || (() => {});
		function draw() {
	...
	var FIELD_SPECS = [
		{
			id: "intro-objective",
			get: () => state.intro.objective,
			set: (v) => {
				state.intro.objective = v;
			}
		},
		{
			id: "intro-scope",
			get: () => state.intro.scope,
			set: (v) => {
				state.intro.scope = v;
			}
		},
		{
			id: "m-approach",
			evt: "change",
			get: () => state.methodology.approach,
			set: (v) => {
				state.methodology.approach = v;
			}
		},
		{
			id: "m-unit",
			evt: "change",
			get: () => state.methodology.unit,
			set: (v) => {
				state.methodology.unit = v;
			}
		},
		{
			id: "m-model",
			get: () => state.methodology.modelType,
			set: (v) => {
				state.methodology.modelType = v;
			}
		},
		{
			id: "m-schedLevel",
			evt: "change",
			get: () => state.methodology.scheduleLevel,
			set: (v) => {
				state.methodology.scheduleLevel = v;
			}
		},
		{
			id: "m-schedClass",
			evt: "change",
			get: () => state.methodology.scheduleClass,
			set: (v) => {
				state.methodology.scheduleClass = v;
			}
		},
		{
			id: "m-levelNotes",
			get: () => state.methodology.levelNotes,
			set: (v) => {
				state.methodology.levelNotes = v;
			}
		},
		{
			id: "c-rule",
			get: () => state.codification.rule,
			set: (v) => {
				state.codification.rule = v;
			}
		},
		{
			id: "c-pattern",
			get: () => state.codification.activityIdPattern,
			set: (v) => {
				state.codification.activityIdPattern = v;
			}
		},
		{
			id: "cal-notes",
			get: () => state.calendar.notes,
			set: (v) => {
				state.calendar.notes = v;
			}
		},
		{
			id: "d-method",
			evt: "change",
			get: () => state.durationEstimating.method,
			set: (v) => {
				state.durationEstimating.method = v;
			}
		},
		{
			id: "d-contingency",
			get: () => state.durationEstimating.contingencyBasis,
			set: (v) => {
				state.durationEstimating.contingencyBasis = v;
			}
		},
		{
			id: "d-notes",
			get: () => state.durationEstimating.notes,
			set: (v) => {
				state.durationEstimating.notes = v;
			}
		},
		{
			id: "cp-method",
			get: () => state.criticalPath.methodology,
			set: (v) => {
				state.criticalPath.methodology = v;
			}
		},
		{
			id: "cp-tool",
			get: () => state.criticalPath.tool,
			set: (v) => {
				state.criticalPath.tool = v;
			}
		},
		{
			id: "cp-threshold",
			numeric: true,
			get: () => state.criticalPath.nearCriticalThresholdDays,
			set: (v) => {
				state.criticalPath.nearCriticalThresholdDays = v;
			}
		},
		{
			id: "cp-float",
			get: () => state.criticalPath.floatOwnership,
			set: (v) => {
				state.criticalPath.floatOwnership = v;
			}
		},
		{
			id: "pm-method",
			evt: "change",
			get: () => state.performanceMeasurement.method,
			set: (v) => {
				state.performanceMeasurement.method = v;
			}
		},
		{
			id: "pm-freq",
			get: () => state.performanceMeasurement.updateFrequency,
			set: (v) => {
				state.performanceMeasurement.updateFrequency = v;
			}
		},
		{
			id: "pm-rules",
			get: () => state.performanceMeasurement.evmRules,
			set: (v) => {
				state.performanceMeasurement.evmRules = v;
			}
		},
		{
			id: "res-pct",
			numeric: true,
			get: () => state.scheduleReserve.pct,
			set: (v) => {
				state.scheduleReserve.pct = v;
			}
		},
		{
			id: "res-basis",
			get: () => state.scheduleReserve.basisText,
			set: (v) => {
				state.scheduleReserve.basisText = v;
			}
		},
		{
			id: "res-gov",
			get: () => state.scheduleReserve.governance,
			set: (v) => {
				state.scheduleReserve.governance = v;
			}
		},
		{
			id: "cc-process",
			get: () => state.changeControl.process,
			set: (v) => {
				state.changeControl.process = v;
			}
		},
		{
			id: "cc-threshold",
			numeric: true,
			get: () => state.changeControl.baselineChangeThresholdPct,
			set: (v) => {
				state.changeControl.baselineChangeThresholdPct = v;
			}
		},
		{
			id: "cc-chain",
			get: () => state.changeControl.approvalChain,
			set: (v) => {
				state.changeControl.approvalChain = v;
			}
		},
		{
			id: "ap-preparedBy",
			get: () => state.approval.preparedBy,
			set: (v) => {
				state.approval.preparedBy = v;
			}
		},
		{
			id: "ap-preparedRole",
			get: () => state.approval.preparedRole,
			set: (v) => {
				state.approval.preparedRole = v;
			}
		},
		{
			id: "ap-reviewedBy",
			get: () => state.approval.reviewedBy,
			set: (v) => {
				state.approval.reviewedBy = v;
			}
		},
		{
			id: "ap-approvedBy",
			get: () => state.approval.approvedBy,
			set: (v) => {
				state.approval.approvedBy = v;
			}
		},
		{
			id: "ap-approvalDate",
			get: () => state.approval.approvalDate,
			set: (v) => {
				state.approval.approvalDate = v;
			}
		}
	];
	function wireScalarFields() {
	...
	function renderCalendarChips() {
		...
		box.innerHTML = DAY_OPTIONS.map((d) => {
			return "<span class=\"chip" + (state.calendar.workDays.indexOf(d) !== -1 ? " active" : "") + "\" data-day=\"" + d + "\">" + d + "</span>";
		}).join("");
		box.querySelectorAll("[data-day]").forEach((chip) => {
			chip.addEventListener("click", () => {
				const d = chip.dataset.day;
				const i = state.calendar.workDays.indexOf(d);
				if (i === -1) state.calendar.workDays.push(d);
				else state.calendar.workDays.splice(i, 1);
				renderCalendarChips();
				onDirty();
			});
		});
	...
	function renderTables() {
		renderEditableTable(document.getElementById("cal-holidays-table"), state.calendar.holidays, [{
			key: "date",
			label: "Fecha",
			type: "date",
			width: "140px"
			...
			addLabel: "+ Agregar feriado",
			onChange: onDirty
		});
		renderEditableTable(document.getElementById("mil-table"), state.milestones, [
			{
				key: "name",
				label: "Hito",
				type: "text",
		...
		renderEditableTable(document.getElementById("roles-table"), state.roles, [
			{
				key: "role",
				label: "Rol",
				type: "text",
			...
			},
			{
				...
				type: "text",
				width: "18%",
				list: "gpi-obs-people-sp"
			},
			{
				key: "responsibility",
				label: "Responsabilidad frente al cronograma",
				type: "textarea"
			},
			{
				key: "raci",
		...
		renderEditableTable(document.getElementById("rep-table"), state.reportingFormats, [
			{
				key: "name",
		...
		renderSimpleList(document.getElementById("assum-list"), state.assumptions, {
			addLabel: "+ Agregar supuesto",
			placeholder: "Ej. Los proveedores confirman sus lead times...",
			onChange: onDirty
		});
		renderSimpleList(document.getElementById("excl-list"), state.exclusions, {
			addLabel: "+ Agregar exclusión",
			placeholder: "Ej. No incluye operación posterior a la entrega...",
			onChange: onDirty
		});
		renderWorkingTimes();
		renderThresholds();
	...
	function renderWorkingTimes() {
		...
		const hoursEl = document.getElementById("cal-hours");
		if (hoursEl) hoursEl.value = String(hours);
		box.querySelectorAll("[data-wt]").forEach((el) => {
			el.addEventListener("input", () => {
				wt[+el.dataset.wt][el.dataset.k] = el.value;
				renderWorkingTimes();
				onDirty();
			});
		});
		box.querySelectorAll("[data-wtdel]").forEach((btn) => {
			btn.addEventListener("click", () => {
	...
	function bandOps(higherIsBetter) {
	...
	function renderThresholds() {
		...
		box.querySelectorAll("[data-thr]").forEach((el) => {
			...
			el.addEventListener(evt, () => {
				const idx = +el.dataset.thr, t = arr[idx], k = el.dataset.k;
				const value = el.value;
				if (k === "greenValue" || k === "redValue") t[k] = Number(value);
				else if (k === "higherIsBetter") {
					t.higherIsBetter = value === "true";
					renderThresholds();
					onDirty();
					return;
				} else t[k] = value;
		...
		box.querySelectorAll("[data-thrdel]").forEach((btn) => {
			btn.addEventListener("click", () => {
				arr.splice(+btn.dataset.thrdel, 1);
				renderThresholds();
				onDirty();
			});
		});
	...
	function diffDaysIso(a, b) {
		if (!a || !b) return 0;
		const da = /* @__PURE__ */ new Date(a + "T00:00:00"), db = /* @__PURE__ */ new Date(b + "T00:00:00");
	...
	function updateMetaPanels() {
		...
		if (lodPanel) {
			const LVL = {
				"1": "Nivel 1 · Ejecutivo/Master (hitos)",
				"2": "Nivel 2 · Gerencial/Resumen (fases)",
				"3": "Nivel 3 · Publicación/Control (paquetes)",
				"4": "Nivel 4 · Ejecución/Detallado (actividades)",
				"5": "Nivel 5 · Trabajo/Detalle fino (tareas)"
			};
			const CLS = {
				"5": "Clase 5 · Definición 0–2% (screening de concepto)",
				"4": "Clase 4 · Definición 1–15% (factibilidad)",
				"3": "Clase 3 · Definición 10–40% (presupuesto/autorización)",
				"2": "Clase 2 · Definición 30–70% (control / licitación)",
				"1": "Clase 1 · Definición 70–100% (licitación/oferta)"
			};
			const lv = state.methodology.scheduleLevel, cl = state.methodology.scheduleClass;
			lodPanel.innerHTML = "<b>Detalle declarado (RP 27R-03):</b><div class=\"chips\"><span class=\"chip-stat\">" + esc(lv ? LVL[lv] : "Nivel sin definir") + "</span><span class=\"chip-stat\">" + esc(cl ? CLS[cl] : "Clase sin definir") + "</span></div>";
		}
		...
		if (hasGpi && window.GPI.util && wbs && wbs.nodes) {
			...
			resPanel.innerHTML = "<b>Duración base de la EDT:</b> " + span + " días (" + esc(roll2.minStart || "—") + " → " + esc(roll2.maxEnd || "—") + ")<br><b>Reserva calculada:</b> +" + reserveDays + " días<br><b>Fecha objetivo con reserva:</b> " + esc(target || "—");
		} else resPanel.innerHTML = "Vincula la EDT para calcular automáticamente los días de reserva y la fecha objetivo a partir del % indicado.";
		const rolesPanel = document.getElementById("rolesRaciInfoPanel");
		if (hasGpi && window.GPI.util && wbs && wbs.nodes) {
			const cov = window.GPI.util.raciCoverage(raci || {}, wbs);
			rolesPanel.innerHTML = "<b>Cobertura RACI actual:</b> " + cov.withR + "/" + cov.total + " paquetes de trabajo con Responsable ('R') asignado." + (cov.withoutR.length ? " Completa la Matriz RACI para los paquetes restantes antes de cerrar este plan." : " Todos los paquetes tienen Responsable — buena base para medir el desempeño del cronograma.");
		} else rolesPanel.innerHTML = "Vincula la EDT y la Matriz RACI para ver aquí la cobertura de Responsables por paquete de trabajo.";
	}
	function currentAudit() {
		if (typeof window.GPI === "undefined" || !window.GPI.util) return null;
		return window.GPI.util.schedulePlanAudit(state);
	}
	var STATE_COLORS = {
		verde: "#00c2a8",
		ambar: "#ff9f1c",
	...
	function updateSidebar() {
		...
		const pctEl = document.getElementById("sbPct"), fillEl = document.getElementById("sbBarFill"), labelEl = document.getElementById("sbStateLabel"), boxEl = document.getElementById("sbChecklist");
		if (!audit) {
			pctEl.textContent = "—";
			pctEl.style.color = "";
		...
		pctEl.textContent = audit.pct + "%";
		pctEl.style.color = color;
		fillEl.style.width = audit.pct + "%";
		...
		boxEl.innerHTML = order.map((cat) => {
			...
			return "<div class=\"chk-cat\"><div class=\"chk-cat-h\">" + esc(cat) + "<span>" + okN + "/" + items.length + "</span></div>" + items.map((i) => "<div class=\"chk-item" + (i.ok ? " ok" : "") + "\"><span class=\"chk-dot\">" + (i.ok ? "✓" : "○") + "</span>" + esc(i.label) + "</div>").join("") + "</div>";
		}).join("");
	}
	function onDirty() {
		updateMetaPanels();
	...
	function fullRender() {
		state.methodology.tool = FIXED_TOOL;
	...
	function init() {
		...
		document.getElementById("btnAddPeriod").addEventListener("click", () => {
			state.calendar.workingTimes.push({
				from: "13:00",
				to: "17:00"
			});
			renderWorkingTimes();
		...
		document.getElementById("btnSample").addEventListener("click", async () => {
			if (await showConfirm("Esto reemplazará el plan actual por el ejemplo DISTRIB+ S.A. ¿Continuar?", "Cargar ejemplo")) {
				state = normalizeState(sampleState());
				fullRender();
		...
		document.getElementById("btnReset").addEventListener("click", async () => {
			if (await showConfirm("Esto borrará el plan actual y comenzará uno en blanco. ¿Continuar?", "Nuevo plan")) {
				state = normalizeState(defaultState());
				fullRender();
				setStatus("Nuevo plan en blanco.");
			}
		});
	}
	...
	document.addEventListener("DOMContentLoaded", function gpiBridge() {
		...
		function refreshPeopleList() {
			...
			let dl = document.getElementById("gpi-obs-people-sp");
			if (!dl) {
				dl = document.createElement("datalist");
				dl.id = "gpi-obs-people-sp";
	...
	function buildReport() {
		function kv(label, val) {
			return "<tr><td>" + esc(label) + "</td><td>" + ((val || val === 0) && String(val).trim() ? esc(String(val)) : "<span class=\"rep-note\">—</span>") + "</td></tr>";
		}
		function list(arr) {
		...
		const APPROACH_M = {
			predictivo: "Predictivo (planificación en cascada)",
			hibrido: "Híbrido",
			agil: "Ágil / iterativo"
		};
		const DUR_M = {
			juicio_expertos: "Juicio de expertos",
			analoga: "Estimación análoga",
			parametrica: "Estimación paramétrica",
			tres_valores: "Tres valores (PERT: optimista / probable / pesimista)"
		};
		const PM_M = {
			"0_100": "Regla 0/100 (hitos de control)",
			"50_50": "Regla 50/50",
			fisico_ponderado: "% de avance físico ponderado",
			hitos_ponderados: "Hitos ponderados (curva de avance)"
		};
		const TYPE_M = {
			interno: "Interno",
		...
		const s = state;
		let body = "";
		body += "<h2>1. Objetivo y alcance del plan</h2><table class=\"rep-kv\">" + kv("Objetivo", s.intro.objective) + kv("Alcance de aplicación", s.intro.scope) + "</table>";
		body += "<h2>2. Metodología, herramienta y nivel del cronograma</h2><table class=\"rep-kv\">" + kv("Enfoque de desarrollo", APPROACH_M[s.methodology.approach] || s.methodology.approach) + kv("Herramienta de programación", s.methodology.tool) + kv("Unidad de duración", s.methodology.unit) + kv("Nivel del cronograma (AACE RP 37R-06)", s.methodology.scheduleLevel ? "Nivel " + s.methodology.scheduleLevel : "") + kv("Clase del cronograma (AACE RP 27R-03)", s.methodology.scheduleClass ? "Clase " + s.methodology.scheduleClass : "") + kv("Justificación de nivel y clase", s.methodology.levelNotes) + kv("Tipo de modelo de programación", s.methodology.modelType) + "</table>";
		body += "<h2>3. Codificación de actividades</h2><table class=\"rep-kv\">" + kv("Regla de codificación", s.codification.rule) + kv("Patrón de identificador", s.codification.activityIdPattern) + "</table>";
		...
		body += "<h2>4. Calendario del proyecto</h2><table class=\"rep-kv\">" + kv("Días laborables", (s.calendar.workDays || []).join(", ")) + kv("Horas por día", s.calendar.hoursPerDay) + kv("Horarios de trabajo", wt) + kv("Feriados / días no laborables", hol) + kv("Notas del calendario", s.calendar.notes) + "</table>";
		body += "<h2>5. Estimación de duraciones</h2><table class=\"rep-kv\">" + kv("Método principal", DUR_M[s.durationEstimating.method] || s.durationEstimating.method) + kv("Base de contingencia", s.durationEstimating.contingencyBasis) + kv("Notas", s.durationEstimating.notes) + "</table>";
		body += "<h2>6. Ruta crítica</h2><table class=\"rep-kv\">" + kv("Metodología", s.criticalPath.methodology) + kv("Herramienta / verificación", s.criticalPath.tool) + kv("Umbral de ruta casi crítica", s.criticalPath.nearCriticalThresholdDays ? s.criticalPath.nearCriticalThresholdDays + " días de holgura total" : "") + kv("Propiedad de la holgura", s.criticalPath.floatOwnership) + "</table>";
		const th = s.controlThresholds || [];
		body += "<h2>7. Umbrales de control</h2>";
		body += th.length ? "<table><tr><th>Métrica</th><th style=\"width:11%\">Verde hasta</th><th style=\"width:11%\">Rojo desde</th><th>Acción al exceder el umbral</th></tr>" + th.map((t) => {
		...
		}).join("") + "</table>" : "<p class=\"rep-note\">— No registrado —</p>";
		body += "<h2>8. Medición del desempeño (valor ganado)</h2><table class=\"rep-kv\">" + kv("Método de medición", PM_M[s.performanceMeasurement.method] || s.performanceMeasurement.method) + kv("Reglas de valor ganado", s.performanceMeasurement.evmRules) + kv("Frecuencia de actualización", s.performanceMeasurement.updateFrequency) + "</table>";
		const mil = (s.milestones || []).filter((m) => (m.name || "").trim());
		body += "<h2>9. Hitos del cronograma</h2>";
		body += mil.length ? "<table><tr><th>Hito</th><th style=\"width:11%\">Fecha</th><th style=\"width:12%\">Tipo</th><th style=\"width:11%\">Restricción</th><th>Notas</th></tr>" + mil.map((m) => "<tr><td>" + esc(m.name) + "</td><td class=\"num\">" + repDate(m.date) + "</td><td>" + esc(TYPE_M[m.type] || m.type || "—") + "</td><td class=\"num\">" + esc(m.constraint || "—") + "</td><td>" + esc(m.notes || "") + "</td></tr>").join("") + "</table>" : "<p class=\"rep-note\">— No registrado —</p>";
		body += "<h2>10. Reserva de cronograma</h2><table class=\"rep-kv\">" + kv("Reserva", s.scheduleReserve.pct ? s.scheduleReserve.pct + "% de la duración total" : "") + kv("Base de estimación", s.scheduleReserve.basisText) + kv("Gobernanza de consumo", s.scheduleReserve.governance) + "</table>";
		const roles = (s.roles || []).filter((r) => (r.role || "").trim());
		body += "<h2>11. Roles y responsabilidades sobre el cronograma</h2>";
		body += roles.length ? "<table><tr><th style=\"width:22%\">Rol</th><th style=\"width:17%\">Persona / referencia</th><th>Responsabilidad</th><th style=\"width:7%\">RACI</th></tr>" + roles.map((r) => "<tr><td>" + esc(r.role) + "</td><td>" + esc(r.person || "—") + "</td><td>" + esc(r.responsibility) + "</td><td class=\"num\" style=\"text-align:center\">" + esc(r.raci || "—") + "</td></tr>").join("") + "</table>" : "<p class=\"rep-note\">— No registrado —</p>";
		const reps = (s.reportingFormats || []).filter((r) => (r.name || "").trim());
		body += "<h2>12. Reportes y formatos de seguimiento</h2>";
		body += reps.length ? "<table><tr><th>Reporte</th><th style=\"width:13%\">Frecuencia</th><th style=\"width:24%\">Audiencia</th><th style=\"width:24%\">Herramienta</th></tr>" + reps.map((r) => "<tr><td>" + esc(r.name) + "</td><td>" + esc(r.frequency || "—") + "</td><td>" + esc(r.audience || "—") + "</td><td>" + esc(r.tool || "—") + "</td></tr>").join("") + "</table>" : "<p class=\"rep-note\">— No registrado —</p>";
		body += "<h2>13. Control de cambios del cronograma</h2><table class=\"rep-kv\">" + kv("Proceso", s.changeControl.process) + kv("Umbral de re-baselinado", s.changeControl.baselineChangeThresholdPct ? s.changeControl.baselineChangeThresholdPct + "% de variación sobre la línea base" : "") + kv("Cadena de aprobación", s.changeControl.approvalChain) + "</table>";
		...
		body += "<div class=\"rep-sign\"><div class=\"box\"><b>" + esc(s.approval.approvedBy || "Aprobado por") + "</b><div class=\"r\">Aprobación — Fecha: " + repDate(s.approval.approvalDate) + "</div></div><div class=\"box\"><b>" + esc(s.approval.reviewedBy || "Revisado por") + "</b><div class=\"r\">Revisión · Preparó: " + esc(s.approval.preparedBy || "—") + (s.approval.preparedRole ? " (" + esc(s.approval.preparedRole) + ")" : "") + "</div></div></div>";
