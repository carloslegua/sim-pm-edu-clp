(function() {
	...
	function renderCoverage() {
		...
		if (!emergent.length) h += "<div class=\"note ok\">No hay requisitos emergentes: todos trazan a un RAN del Acta.</div>";
			...
			emergent.forEach((it) => {
				h += "<tr><td class=\"mono\">" + esc(it.code) + "</td><td>" + esc(it.text) + "</td><td>" + stkChip(it) + "</td><td style=\"text-align:right\"><button class=\"btn sm\" onclick=\"promoteToRan('" + it.id + "')\">↥ Promover a RAN del Acta</button></td></tr>";
			});
	...
	function exportJSON() {
		const payload = {
			kind: "gpi.requirements/v1",
			title: gpiOn() && GPI.meta() && GPI.meta().name || "Recopilar Requisitos",
		...
		a.download = "gpi_requirements.json";
		a.click();
	}
	function importJSON(ev) {
		...
		r.onload = () => {
			try {
				...
				applyData(obj && obj.kind === "gpi.requirements/v1" && obj.data ? obj.data : obj);
				userEdited = true;
				save();
				flash();
				renderAll();
				...
				showToast("Archivo inválido: no pude leer ese .json como módulo de Requisitos.");
			}
		};
		r.readAsText(f);
		ev.target.value = "";
