...
export interface PlanFacts {
  ...
  quality: { has: boolean; state: AreaState; needing: number; verified: number; checks: number; coqTotal: number; exec?: { inspections: number; ncrOpen: number; ncrOverdue: number; ncrCritical: number } };
  comms: { has: boolean; state: AreaState; items: number; covered: number; stakeholders: number; closeUncovered: number; exec?: { logged: number; periodicSilent: number; closeSilent: number } };
  procurement: { has: boolean; state: AreaState; items: number; total: number; late: number; soon: number; asOf: string; exec?: { payments: number; overpaid: number; claimsOpen: number; claimsStale: number } };
