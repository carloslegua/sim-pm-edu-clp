...
// ---------- ejemplo DISTRIB+ ----------
interface SampleRisk extends Partial<Risk> { wbs: string[]; }
const SAMPLE_PLAN = normalizePlan({
...
function sampleRisks(): Risk[] {
  return SAMPLE_RISKS.map((s, i) => {
    // conectado: se empareja por Código EDT con la EDT real (lo que no existe se omite); independiente: ids de muestra.
    const ids = s.wbs.map((c) => { const l = leaves.find((x) => x.code === c); return l ? l.id : ""; }).filter(Boolean);
    return normalizeRisk({ ...s, id: "rk" + (i + 1), identifiedOn: "2026-07-06", wbsIds: ids }, "rk" + (i + 1));
  });
}
...
const findingsOf = (r: Risk): Finding[] => riskFindings(r, plan, { today: todayISO(), costBase, leafIds: leaves.map((l) => l.id) });
