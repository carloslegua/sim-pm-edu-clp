...
import { newVersionProblems, suggestNextVersion } from "../../shared/requirements-baseline";
...
// La línea base incluye el enunciado, la EDT y su diccionario (shared/scope-baseline.ts) y archiva cada versión al fijar la siguiente.
type ScopeBaseline = ScopeBaselineData;
...
function normalize(d: any): ScopeState {
  ...
  if (d.baseline) s.baseline = normalizeScopeBaseline(d.baseline);   // tolerante: lo guardado antes no trae EDT, motivo ni historial
  reCode(s);
  return s;
}
...
function confirmModal(title: string, msg: string, okText?: string, danger?: boolean): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    card.innerHTML = '<h3 id="modalCardTitle">' + esc(title) + '</h3><p>' + esc(msg) + '</p>'
...
// El borrador del formulario de «nueva versión» sobrevive a los re-dibujos (otra pestaña edita la EDT y esta se refresca mientras se escribe).
const NV_IDS = ["nv_ver", "nv_date", "nv_appr", "nv_reason"];
function renderBaseline(a: GpiCore.ScopeAuditResult): void {
  const b = state.baseline, host = $("baselineHost"), st = $("baselineState");
  if (b.frozen) {
    const wbsNow = mod("wbs"), drift = scopeDriftOf(b, state, wbsNow), draft = NV_IDS.map((id) => { const el = document.getElementById(id) as HTMLInputElement | null; return el ? el.value : undefined; });
    ...
    let h = '<div class="muted">Línea base <b>v' + esc(b.version) + '</b> congelada el <b>' + esc(b.date || "—") + '</b> por <b>' + esc(b.approver || "—") + '</b>' + (b.reason ? ' · motivo: ' + esc(b.reason) : '') + '. '
      + 'Contiene ' + (((b.snapshot?.deliverables as unknown[]) || []).length) + ' entregable(s)' + (drift.wbsInBaseline ? ', el enunciado y <b>' + nEdt + ' elemento(s) de la EDT con su diccionario</b>' : '') + '. '
      + 'Los cambios posteriores se gestionan por Control Integrado de Cambios y solo una <b>nueva versión</b> actualiza esta referencia.</div>';
    ...
    // trabajo en edición frente a lo aprobado
    if (drift.total) {
      h += '<div class="card-sub" style="margin-top:10px"><b>Trabajo en edición distinto de lo aprobado (v' + esc(b.version) + '):</b><ul class="warn-list" style="margin-top:6px">'
        + (drift.enunciadoChanged ? '<li><span class="tag a">revisar</span>El enunciado (entregables, supuestos, restricciones, exclusiones o alcances) cambió desde la aprobación.</li>' : '')
        + drift.wbsChanges.slice(0, 12).map((c) => '<li><span class="tag a">EDT</span><b>' + esc(c.code || "—") + '</b> ' + esc(c.name) + ' — ' + esc(c.kind) + ': ' + esc(c.detail) + '</li>').join("")
        + (drift.wbsChanges.length > 12 ? '<li class="muted">…y ' + (drift.wbsChanges.length - 12) + ' cambio(s) más en la EDT.</li>' : '')
        + '</ul><div class="muted" style="font-size:12px">Lo aprobado sigue siendo la instantánea; estos cambios son trabajo en edición hasta que se apruebe una nueva versión.</div></div>';
    ...
    // nueva versión (archiva la vigente)
    const dv = (i: number, def: string): string => escAttr(draft[i] !== undefined ? draft[i] : def);
    h += '<div style="margin-top:14px;border-top:1px solid var(--line);padding-top:12px"><b>Nueva versión de la línea base</b> <span class="muted" style="font-size:12px">— archiva la v' + esc(b.version) + ' completa (enunciado, EDT y diccionario) antes de establecer la siguiente.</span>'
      ...
      + '<div style="margin-top:8px"><button class="btn primary sm" id="btnNewVer">🔒 Fijar nueva versión</button></div></div>';
    // historial
    if (b.history.length) {
      h += '<div style="margin-top:14px;border-top:1px solid var(--line);padding-top:12px"><b>Historial de versiones (' + b.history.length + ')</b>'
        + b.history.slice().reverse().map((v) => {
          const nd = v.snapshot && Array.isArray(v.snapshot.deliverables) ? (v.snapshot.deliverables as unknown[]).length : 0, wv = v.snapshot && v.snapshot.wbs ? v.snapshot.wbs : null, codes = wv ? wbsScopeCodes(wv) : {};
          return '<details class="lb-hist" style="border:1px solid var(--line);border-radius:10px;padding:8px 12px;margin-top:8px"><summary style="cursor:pointer;font-size:12.5px"><b>v' + esc(v.version) + '</b> · aprobada ' + esc(v.date || "—") + ' por ' + esc(v.approver || "—") + ' · ' + nd + ' entregable(s)' + (wv ? ' · ' + (Object.keys(wv.nodes).length - 1) + ' elemento(s) de la EDT' : ' · sin EDT (anterior a incluirla)') + ' · sustituida el ' + esc(v.supersededOn || "—") + '</summary>'
            + '<div class="muted" style="font-size:12px;margin:6px 0"><b>Motivo:</b> ' + esc(v.reason || "—") + '</div>'
            + (wv ? '<table style="width:100%;font-size:12px"><thead><tr><th>Cód.</th><th>Elemento de la EDT</th><th>Criterio de aceptación</th></tr></thead><tbody>' + Object.keys(codes).sort((x, y) => codes[x].localeCompare(codes[y], undefined, { numeric: true })).map((id) => '<tr><td>' + esc(codes[id]) + '</td><td>' + esc(wv.nodes[id].name) + '</td><td>' + esc(wv.nodes[id].acceptance || "—") + '</td></tr>').join("") + '</tbody></table>' : '')
            + '</details>';
        }).join("") + '</div>';
    }
    host.innerHTML = h;
    ($("btnNewVer") as HTMLElement).onclick = () => {
    ...
    host.innerHTML = '<div class="muted" style="margin-bottom:10px">Cuando el alcance esté acordado, <b>congela la línea base</b>: guarda una copia inmutable del <b>enunciado</b> (entregables, supuestos, restricciones y exclusiones), de la <b>EDT</b> y de su <b>diccionario</b> (descripción del trabajo, criterio de aceptación y esfuerzo continuo de cada elemento). Sirve de referencia para medir el <i>scope creep</i>; el costo y las fechas tienen sus propias líneas base.</div>'
      + (mod("wbs") ? "" : '<div class="empty-note" style="margin-bottom:10px">ⓘ No hay una EDT en el proyecto activo: la línea base se congelaría sin ella. Arma la EDT en WBS Builder antes de congelar para que quede en lo aprobado.</div>')
