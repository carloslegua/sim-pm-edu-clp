...
document.addEventListener("DOMContentLoaded", function gpiBridge() {
  ...
  // Versión del organigrama que esta pestaña cargó (GPI.openSession, en pull()).
  ...
  function pull(): void {
    ...
    session = GPI.openSession("obs"); // versión que esta pestaña carga (mismo instante en que lee el dato)
  ...
  function push(): boolean {
    ...
    const r = pushWithSession(GPI, "obs", "El organigrama", { rootId, idCounter, nodes }, { course: courseEl.value }, session, { setStatus, onStale: markProjectStale });
    session = r.session;
    return r.ok;
  }
