...
document.addEventListener("DOMContentLoaded", function gpiBridge() {
  ...
  // Versión del plan que esta pestaña cargó (GPI.openSession, en pull()).
  ...
  function pull(): void {
    ...
    session = window.GPI!.openSession("schedulePlan"); // versión del plan que esta pestaña carga
    ...
    if (mod) window.GPI!.rebaseSession(session, state); // el dato ya normalizado, no el crudo
    fullRender();
    refreshPeopleList();
  ...
  function push(): boolean {
    ...
    const r = pushWithSession(window.GPI!, "schedulePlan", "El plan de cronograma", state, { name: titleEl.value, course: courseEl.value }, session, { setStatus, onStale: markProjectStale });
    session = r.session;
    return r.ok;
  }
