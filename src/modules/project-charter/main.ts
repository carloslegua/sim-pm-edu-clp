...
function sampleState(): CharterState {
  return {
    ...
    objectives: [
      ...
      { dim: "Cronograma", objective: "Concluir el proyecto en 13 meses (julio de 2026 – julio de 2027)", criteria: "Entrega final a más tardar el 23/07/2027 (273 días laborables); SPI ≥ 0.95 en los cortes de control" },
    ...
    milestones: [
      ...
      { name: "Fin de Ingeniería y Diseño", date: "2026-09-30" },
      { name: "Fin de Procura (estructuras metálicas en obra)", date: "2026-10-19" },
      { name: "Permisos y licencias municipales aprobados", date: "2026-11-11" },
      { name: "Fin de Construcción", date: "2027-06-30" },
      { name: "Entrega final y acta de cierre", date: "2027-07-23" }
    ],
    budget: {
      amount: "8500000",
      currency: "USD",
    ...
    constraints: [
      ...
      "Fecha límite de entrega: 23/07/2027, antes de la campaña logística de fin de año de 2027.",
...
function renderObjectives(): void {
  ...
  tb.innerHTML = state.objectives.map((o, i) => {
    ...
    return '<tr>'
      ...
      + '<td class="rt-del"><button data-del-obj="' + i + '" title="Eliminar" aria-label="Eliminar objetivo">🗑</button></td></tr>';
  }).join("");
...
function renderMilestones(): void {
  ...
  tb.innerHTML = state.milestones.map((m, i) => {
    return '<tr>'
      ...
      + '<td class="rt-del"><button data-del-mil="' + i + '" title="Eliminar" aria-label="Eliminar hito">🗑</button></td></tr>';
  }).join("");
...
function renderStakeTable(): void {
  ...
  tb.innerHTML = state.stakeholders.map((s, i) => {
    return '<tr>'
      ...
      + '<td class="rt-del"><button data-del-st="' + i + '" title="Eliminar" aria-label="Eliminar interesado">🗑</button></td></tr>';
  }).join("");
...
function renderApprovalReq(): void {
  ...
  tb.innerHTML = state.approvalRequirements.map((r, i) => {
    return '<tr>'
      ...
      + '<td class="rt-del"><button data-del-ar="' + i + '" title="Eliminar" aria-label="Eliminar requisito de aprobación">🗑</button></td></tr>';
  }).join("");
...
function renderSponsors(): void {
  ...
  tb.innerHTML = state.sponsors.map((s, i) => {
    return '<tr>'
      ...
      + '<td class="rt-del"><button data-del-sp="' + i + '" title="Eliminar" aria-label="Eliminar patrocinador">🗑</button></td></tr>';
  }).join("");
...
function renderList(key: ListKey): void {
  ...
  host.innerHTML = state[key].map((txt, i) => {
    return '<div class="sl-row">'
      ...
      + '<button class="row-del" data-del-list="' + key + '" data-i="' + i + '" title="Eliminar" aria-label="Eliminar elemento">🗑</button></div>';
...
function renderRequirements(): void {
  ...
  host.innerHTML = state.requirements.map((r, i) => {
    return '<div class="sl-row">'
      ...
      + '<button class="row-del" data-del-req="' + i + '" title="Eliminar" aria-label="Eliminar requisito">🗑</button></div>';
...
async function importMilestonesFromWbs(): Promise<void> {
  ...
  // La EDT efectiva: con cronograma, las fechas del CPM (no las manuales de la EDT, que no se actualizan).
