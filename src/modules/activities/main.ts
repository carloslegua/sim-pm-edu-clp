...
// para nunca escribir estas actividades sobre un proyecto distinto que
...
// resumen), e Id CONSECUTIVO SIN SALTOS para todas las demás filas (fases,
// paquetes, actividades E HITOS). Es la única fuente de numeración: la
// tabla, el reporte y la plantilla exportada lo comparten para que nunca se
// desalineen. Este Id debe coincidir fila por fila con el Task ID que
// asigna MS Project al mismo cronograma (ahí un hito también es una fila
// con su propio ID consecutivo, nunca un hueco) -- por eso NINGUNA fila,
// hitos incluidos, se salta el contador: hacerlo rompería la
// correspondencia 1:1 que el alumno necesita al pegar/exportar esta tabla
// contra MS Project. level = nivel de esquema de MS Project (proyecto=1,
// sus fases=2, …). Los hitos atados a un paquete se listan después de sus
// actividades; los sueltos se intercalan en CUALQUIER posición del listado
// (ver placeLooseMilestones) -- nunca en un bloque aparte ni con
// numeración EDT propia (su código EDT-like es su propio código H1/H2…).
...
const TEMPLATE_HEADERS = ["Id.", "Código EDT", "Paquete de trabajo", "Nombre de la actividad", "Tipo", "Código de hito", "Unidad", "Metrado", "Rendimiento (R)", "N.º de equipos"];
...
// archivo". Antes se llamaba "EDT"; renombrada a "Actividades" a pedido
// explícito del usuario para que el nombre describa el módulo, no la EDT
// (que es la hoja propia de WBS Builder).
const DATA_SHEET_NAME = "Actividades";
...
// Id. = el mismo correlativo consecutivo (fullRows().n) que ya se ve en
// pantalla -- se completa solo para la fila de PAQUETE (referencia, igual
// que Código EDT/Paquete de trabajo). Si duplicas la fila para agregar más
// de una actividad al mismo paquete, las copias conservan el mismo Id.: al
// importar, ese Id. se contrasta contra Código EDT + Paquete de trabajo
// (nunca contra el nombre de la actividad), así que duplicar la fila no
// genera un aviso falso -- ver reconcileImportRows().
...
async function buildTemplateXlsxBlob(): Promise<Blob> {
  ...
  zip.file("xl/workbook.xml",
    ...
    + '<sheets><sheet name="Actividades" sheetId="1" r:id="rId1"/><sheet name="Instrucciones" sheetId="2" r:id="rId2"/></sheets>'
    + '</workbook>');
  ...
  zip.file("xl/worksheets/sheet1.xml", xlsxSheetXml(templateRowModel(), [6, 10, 30, 30, 8, 12, 10, 11, 14, 12], true));
...
function buildTemplateCsv(): string {
  ...
  fullRows().filter((r) => r.kind === "package").forEach((r) => { lines.push([cell(r.n), cell(r.code), cell(r.name || ""), "", "", "", "", "", "", ""].join(";")); });
  return lines.join("\r\n");
}
...
interface ColumnMap { code: number; name: number; id?: number; pkgName?: number; unit?: number; qty?: number; perf?: number; teams?: number; type?: number; milestoneCode?: number; }
// Posición por posición con TEMPLATE_HEADERS. "Paquete de trabajo" SÍ se lee
// (pkgName) -- ver reconcileImportRows(): la EDT (WBS Builder) es la capa
// que manda sobre este módulo, así que además de resolver el paquete por
// Código EDT, se verifica que el nombre del archivo coincida con el nombre
// REAL de ese paquete ahora mismo en WBS Builder -- mismo criterio, y mismo
// motivo, que ya usa Estimar los Costos. "Id." también se lee -- ver
// reconcileImportRows(): verifica, para ese mismo Id., que el Código EDT y
// el Paquete de trabajo del archivo sigan correspondiendo a lo que hay
// AHORA MISMO en esa posición (detecta un archivo desactualizado, p. ej.
// exportado antes de que WBS Builder corriera la numeración) -- aviso, no
// bloqueo, y a propósito NUNCA se contrasta contra el nombre de la
// actividad (que sí puede repetirse en varias filas si se duplicó la fila
// del paquete para más de una actividad).
const TEMPLATE_HEADER_FIELDS: (keyof ColumnMap | null)[] = ["id", "code", "pkgName", "name", "type", "milestoneCode", "unit", "qty", "perf", "teams"];
...
interface ReconcileResult {
  ...
  idMismatches: { id: number; code: string }[];
}
...
function reconcileImportRows(rows: string[][], colMap: ColumnMap): ReconcileResult {
  ...
  leafRows().forEach((l) => { codeToId[l.code] = l.id; codeToName[l.code] = l.name; });
  // Id. -> {code, name} de LO QUE SEA que ocupe esa posición AHORA MISMO
  // (paquete, fase, hito...) -- deliberadamente no se filtra a solo
  // paquetes: si, por ejemplo, se insertó un hito antes de un paquete, ese
  // paquete corre de Id. sin que su propio Código EDT cambie, y la posición
  // vieja pasa a estar ocupada por el hito (con un código H1/H2… muy
  // distinto) -- eso también debe detectarse como drift.
  const currentPkgById = new Map<number, { code: string; name: string }>();
  fullRows().forEach((r) => { currentPkgById.set(r.n, { code: r.code, name: r.name }); });
  ...
  const idMismatches: { id: number; code: string }[] = [];
  rows.forEach((row) => {
    ...
    if (colMap.pkgName != null) {
      ...
      if (fileName && normalizeHeader(fileName) !== normalizeHeader(realName)) {
        packageMismatches.push({ code, fileName, realName });
        return; // la EDT (WBS Builder) manda: no se asume el paquete solo por el código
      }
    }
    if (colMap.id != null) {
      ...
      if (!Number.isNaN(idNum)) {
        ...
        // Solo avisa si esa posición existe pero corresponde a OTRO paquete
        // -- nunca contra el nombre de la actividad (varias filas duplicadas
        // del mismo paquete comparten Id. a propósito, ver templateRowModel()).
        if (current && current.code !== code) idMismatches.push({ id: idNum, code });
      }
    }
  ...
  return { byLeaf, idCounter: n + 1, matched, unmatchedCodes: Array.from(unmatched), milestones, matchedMilestones, milestoneIssues, packageMismatches, idMismatches };
}
...
function markProjectStale(): void {
  ...
  if (banner) {
    banner.innerHTML = "<b>El proyecto activo cambió en otra pestaña.</b> Esta pestaña quedó desactualizada y ya no puede guardar las actividades aquí -- recárgala para seguir trabajando sobre el proyecto activo, o vuelve a activar el proyecto original desde el Panel de Control.";
