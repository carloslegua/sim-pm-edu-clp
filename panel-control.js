		...
		if (sch && (sch.links || []).length) {
			...
			if (ss.ok) {
				...
				if (ss.baselineVersion) {
					...
					scBody += "<div class=\"" + (Math.abs(dv) < .05 ? "ok-note" : "warn-note") + "\" style=\"margin-top:6px\">Línea base <b>" + esc(ss.baselineVersion) + "</b>: " + (Math.abs(dv) < .05 ? "sin desviación" : (dv > 0 ? "+" : "−") + Math.round(Math.abs(dv) * 10) / 10 + " d" + (pc !== null ? " (" + (pc > 0 ? "+" : "−") + Math.round(Math.abs(pc) * 10) / 10 + " %)" : "") + " frente al pronóstico") + "</div>";
