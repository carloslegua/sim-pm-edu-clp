...
describe("Panel_Control.html (migrado a panel-control.js)", () => {
  it("localStorage vacío: ensureSeed crea el proyecto DISTRIB+ y el launcher pinta las 27 tarjetas de módulo en los 7 dominios de PMBOK 8 (23 activas, 4 módulos faltantes)", async () => {
    const dom = await JSDOM.fromURL(base + "Panel_Control.html", { runScripts: "dangerously", resources: "usable" });
    const doc = dom.window.document;
    await esperarHasta(() => doc.querySelectorAll(".mod-card").length === 27 && doc.querySelectorAll("#projSelect option").length === 1, "que el Panel siembre el proyecto y pinte las 27 tarjetas");
