...
describe("Panel_Control.html (migrado a panel-control.js)", () => {
  it("localStorage vacío: ensureSeed crea el proyecto DISTRIB+ y el launcher pinta las 22 tarjetas de módulo (17 activas, 5 'próximamente')", async () => {
    ...
    expect(doc.querySelectorAll(".mod-card.active").length).toBe(17);   // + Gestión de Riesgos (Risk_Register.html), Valor Ganado (Valor_Ganado.html), Control de Cambios (Control_Cambios.html) y Plan para la Dirección (Plan_Direccion.html)
    expect(doc.querySelectorAll(".mod-card.soon").length).toBe(5);
