...
describe("Cost-management.html — escalación por índices (58R-10 / 68R-11)", () => {
  it("ejemplo DISTRIB+: por índices, cuentas y años; escalación financiada P70 = 172,386 y el BAC la suma", async () => {
    ...
    expect(txt(doc, "kEsc")).toBe("$ 172,386"); expect(txt(doc, "kFx")).toBe("$ 0"); expect(txt(doc, "kBAC")).toBe("$ 8,124,386");
    expect(txt(doc, "kEscCap")).toBe("P70 de la simulación");
    const r = txt(doc, "escResults");
    expect(r).toMatch(/Escalación central\$ 125,477/); expect(r).toMatch(/Financiada\$ 172,386/);
    expect(r).toMatch(/P50\$ 157,106/); expect(r).toMatch(/P80\$ 182,075/); expect(r).toMatch(/P90\$ 196,173/);
    // por cuenta (del costo base): Σ = 112,033; de la contingencia (852,000) salen 13,444 al central
    ["Mano de obra", "Materiales", "Equipos", "Subcontratos"].forEach((c) => expect(r).toContain(c));
    expect(r).toMatch(/Mano de obra\$ 2,158,750\$ 43,435/); expect(r).toMatch(/Materiales\$ 2,721,750\$ 26,930/);
    expect(r).toMatch(/2026\$ 4,023,415\$ 32,356/); expect(r).toMatch(/2027\$ 3,076,585\$ 79,677/);
    expect(r).toMatch(/\$ 18,470 corresponde a la contingencia/);
    expect(r).toMatch(/El pronóstico central equivale al P10/);                                   // el retraso del cronograma hace probable escalar más que el central
  ...
  it("segrega la contingencia y el tipo de cambio: 58R-10 pide estimarlos por separado; el riesgo de precio del registro se señala (X14)", async () => {
    ...
    expect(txt(doc, "kEsc")).toBe("$ 172,386"); expect(txt(doc, "kFx")).toBe("$ 170,400");         // 7,100,000 × 30 % × 8 %
  ...
  it("qué escalación se financia: central o un percentil de la simulación; escalar la contingencia se puede apagar", async () => {
    ...
    poner(dom, prov, "central"); expect(txt(doc, "kEsc")).toBe("$ 125,477"); expect(txt(doc, "kEscCap")).toBe("pronóstico central");
    poner(dom, campo(doc, '[data-e="prov"]'), "p90"); expect(txt(doc, "kEsc")).toBe("$ 196,173");
    ...
    expect(txt(doc, "kEsc")).toBe("$ 112,033");                                                       // solo el costo base
